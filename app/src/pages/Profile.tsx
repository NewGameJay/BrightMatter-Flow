  ...
  if (!profile) {
    return (
      <div className="text-center py-16">
        ...
        <p className="text-gray-300">No profile found for address: {address}</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="card text-center">
        ...
        <h1 className="text-2xl font-bold text-white mb-2">Creator Profile</h1>
        <p className="text-gray-300 font-mono text-sm">{address}</p>
      </div>

      {/* VeriScore Display */}
      <div className="card">
        <h2 className="text-xl font-semibold text-white mb-4">VeriScore</h2>
        <div className="text-center">
          <div className="text-6xl font-bold text-flow-blue mb-2">{profile.veriScore.toFixed(2)}</div>
          <div className="text-gray-300 mb-4">Overall Performance Score</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="card text-center">
          ...
          <div className="text-sm text-gray-300">Completed Campaigns</div>
        </div>
        <div className="card text-center">
      ...
      <div className="card">
        ...
        {Object.keys(profile.campaignScores).length > 0 ? (
          <div className="space-y-3">
            {Object.entries(profile.campaignScores).map(([campaignId, score]) => (
              <div key={campaignId} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <div>
                  <div className="font-medium text-white">Campaign {campaignId}</div>
          ...
          <p className="text-gray-300 text-center py-8">No campaign history available.</p>
        )}
      </div>
      ...
      <div className="card">
        ...
        {campaigns.filter(c => !c.paidOut).length > 0 ? (
          ...
          <p className="text-gray-300 text-center py-8">No active campaigns.</p>
        )}
      </div>

      {/* Composability Demo */}
      <div className="card bg-gradient-to-r from-flow-blue/10 to-flow-purple/10">
        <h2 className="text-xl font-semibold text-white mb-4">Composability Demo</h2>
        <p className="text-gray-300 mb-4">
          This profile data is publicly readable on-chain and can be used by other dApps in the Flow ecosystem.
        </p>
        <div className="bg-veri-gray p-4 rounded-lg border border-veri-border">
